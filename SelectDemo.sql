...
# MySQL WHERE with the LIKE operator
 ...
 SELECT *FROM orders;
 SELECT * FROM orders
          WHERE shippedDate BETWEEN '2004-01-01' AND '2004-03-31';
...
-- querry to display employees working in office 1,5,7
SELECT employeeNumber,lastName,firstName,jobTitle , email,officeCode
       FROM employees
...
SELECT * FROM customers WHERE phone =0; #not consider as null
...
SELECT contactLastName ,contactFirstName 
        FROM customers order BY contactLastName,contactFirstName DESC;

SELECT jobTitle ,firstName 
        FROM employees order BY jobTitle, firstName;

SELECT jobTitle ,firstName 
        FROM employees order BY 1 DESC ,2;
...
SELECT orderNumber,productCode,quantityOrdered*priceEach
             FROM orderdetails ORDER BY quantityOrdered*priceEach;
             
             
SELECT orderNumber,productCode,quantityOrdered * priceEach TotalPrice
...
SELECT lastName FROM employees ORDER BY lastName ;
 ...
 SELECT contactLastName,state,city
   FROM customers WHERE state IS NOT NULL
	     ORDER BY contactLastName,state,city;
	     
	     
	      SELECT distinct contactLastName,state,city
   FROM customers WHERE state IS NOT NULL
	     ORDER BY contactLastName,state,city;
# -------------------------------------------------------------------------------
# LIMIT clause
...
SELECT customerName ,country , state,creditlimit   FROM customers WHERE creditlimit>10000
        and( country='USA' or state ='France');
...
DESCRIBE products;
select productCode,productName,MSRP 'Maximum Sale Retail Price'
     FROM products;
     
     
SELECT * FROM employees e;

SELECT e.employeeNumber,e.firstName FROM employee e
